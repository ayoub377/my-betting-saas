...
class TransfermarktClubStaffs(TransfermarktBase):
    ...
    def get_club_staffs(self) -> dict:
        ...
        self.response["name"] = self.get_text_by_xpath(Clubs.Staff.NAME)
        self.response["appointed"] = self.get_text_by_xpath(Clubs.Staff.APPOINTED)
        self.response["contract_expires"] = self.get_text_by_xpath(Clubs.Staff.CONTRACT)
